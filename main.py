...
# Сколько оперативной памяти просит одно видео. ComfyUI держит веса H3 в RAM
# целиком — трансформер 20 ГБ, текст-энкодер 15, VAE 5.5. Замер: ram_avail
# падал с 55.9 до 15.2 ГБ, то есть 40.7 ГБ на ролик.
#
# На 64 ГБ это даёт потолок в ОДНО видео, и так и должно быть.
#
# 20-22.09.2026 пробовали обойти это флагом --fast-disk: он уносит веса на
# NVMe, расход RAM падает до 4 ГБ и потолок поднимается до двух. На стенде
# вышло даже быстрее, но в бою два ролика упёрлись в диск — 358-405 с вместо
# 120-150. Замер на стенде врал: веса лежали в page cache после моих же
# прогонов. Подробности в шапке gen-gpu-override.sh.
#
# Вернёте флаг — этой цифре верить нельзя, она считает RAM, которую флаг не
# тратит. И потолок всё равно обязан остаться 1, иначе два потока снова
# подерутся за диск.
