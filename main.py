...
#
# ВНИМАНИЕ: цифра верна только пока ComfyUI запущен с --fast-disk (см.
# gen-gpu-override.sh). Без флага он держит в RAM все веса H3 разом — это ~41
# ГБ, и замер 20.09.2026 дал 26-40 ГБ на один ролик. С флагом веса
# подтягиваются с NVMe: тот же ролик — 4.0 ГБ, два одновременно уложились в
# 13 ГБ при 49 свободных.
#
# Ставим 10, а не 4: запас на page cache и на то, что длинный ролик просит
# больше короткого. На 64 ГБ это даёт потолок 6, который упирается в число
# карт. Снимете --fast-disk — верните сюда 40, иначе планировщик разрешит два
# видео там, где памяти хватает на одно, и машина уйдёт в своп.
