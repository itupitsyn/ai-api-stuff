...
`/api/edit` правит картинки по инструкции: multipart, поле `files` (от одной до
трёх картинок) и `prompt`.

```bash
curl -F files=@portrait.jpg -F prompt='Change her hair color to copper red.' \
     https://<host>/api/edit
```

Промпт здесь — **инструкция**, а не описание желаемого кадра, как в
`/api/txt2img`. Модель обучена на парах «было → стало», поэтому «сделай волосы
рыжими» работает, а «портрет рыжей женщины» работает хуже.

Несколько картинок — это микс: «возьми женщину со второй и посади за верстак с
первой». Вне правки кадр остаётся прежним: лицо, поза и фон не трогаются.

Модель — Qwen-Image-Edit-2511 в nf4 (15.8 ГиБ) плюс Lightning-LoRA на 4 шага:
правка ~13 с, микс ~22 с, пик 22.2 ГБ. Lightning тянется с HF при первом
старте; не скачался — работаем на 20 шагах, о чём будет строка в логе.

Входные картинки ужимаются до 1152×896 по площади с округлением сторон до
кратности 32 — снимок с телефона в исходном размере карту положит.

Z-Image так не умеет и не научится, пока не выложат веса Omni: второй картинке
в неё физически некуда попасть. Подробности и замеры — в ARCHITECTURE.md.
