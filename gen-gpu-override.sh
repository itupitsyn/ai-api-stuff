...
# --fast-disk — ключевой флаг, от него зависит потолок одновременных видео.
# Веса H3 в сумме весят ~41 ГБ (трансформер 20, текст-энкодер 15, VAE 5.5), в
# 24 ГБ VRAM разом не лезут, и без этого флага ComfyUI держит их все в
# оперативке: замер 20.09.2026 — один ролик съедал 26-40 ГБ RAM. С флагом
# веса подтягиваются с NVMe по мере надобности: тот же ролик — 4.0 ГБ, и при
# этом БЫСТРЕЕ (105 с против 117), потому что машина перестаёт свопить.
#
# Отсюда VIDEO_RAM_BUDGET_GB=10 в main.py и потолок в два видео. Уберёшь
# флаг — потолок останется прежним, а памяти перестанет хватать: вернуть
# бюджет на 40 (см. комментарий там же).
#
